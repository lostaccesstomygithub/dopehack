package me.smokedope2016.dopehack.features.gui;
...
import me.smokedope2016.dopehack.features.gui.items.buttons.ModuleButton;
...
public class DopeHackGui extends Screen {
    private static DopeHackGui oyveyGui;
    private static DopeHackGui INSTANCE;

    static {
        INSTANCE = new DopeHackGui();
    }

    private final ArrayList<Component> components = new ArrayList();

    public DopeHackGui() {
        super(Text.literal("DopeHack"));
        setInstance();
        load();
    }

    public static DopeHackGui getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new DopeHackGui();
        }
        return INSTANCE;
    ...
    public static DopeHackGui getClickGui() {
        return DopeHackGui.getInstance();
    }
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : DopeHack.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 90, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    DopeHack.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
