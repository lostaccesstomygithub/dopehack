...
import me.smokedope2016.dopehack.event.impl.ClientEvent;
...
public class ClickGui
    ...
    public void onSettingChange(ClientEvent event) {
        if (event.getStage() == 2 && event.getSetting().getFeature().equals(this)) {
            if (event.getSetting().equals(this.prefix)) {
                DopeHack.commandManager.setPrefix(this.prefix.getPlannedValue());
                Command.sendMessage("Prefix set to " + Formatting.DARK_GRAY + DopeHack.commandManager.getPrefix());
            }
            DopeHack.colorManager.setColor(this.red.getPlannedValue(), this.green.getPlannedValue(), this.blue.getPlannedValue(), this.hoverAlpha.getPlannedValue());
        }
    }
    ...
    public void onEnable() {
        ...
        mc.setScreen(DopeHackGui.getClickGui());
    }

    @Override
    public void onLoad() {
        DopeHack.colorManager.setColor(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.hoverAlpha.getValue());
        DopeHack.commandManager.setPrefix(this.prefix.getValue());
    }

    @Override
    public void onTick() {
        if (!(ClickGui.mc.currentScreen instanceof DopeHackGui)) {
            this.disable();
        }
    }
